        ...
        <div class="w-full md:w-1/2">
            <form action="{{route('users.accounts.add', ['user_id' => $user->id])}}" class="grid grid-cols-3 gap-2" method="post">
                ...
                <div>
                    ...
                    <select class="input-base" name="code_id">
            ...
            <div class="mt-8 w-full md:w-1/2">
                <table class="w-full table-auto">
                    ...
                    <tr>
                        ...
                        <th class="border px-4 py-2">accounts</th>
                        <th class="border px-4 py-2">Accões</th>
                    </tr>
                    </thead>
                    <tbody>
                    @foreach($user->accounts as $accounts)
                        <tr>
                            <td class="border px-4 py-2">{{ $accounts->id }}</td>
                            <td class="border px-4 py-2">{{ $accounts->code }}</td>

                            <td class="border px-4 py-2 flex justify-between flex-wrap">
                                <form action="{{ route('users.accountss.add', ['user_id'=> $user->id]) }}" method="post">
                                    @csrf
                                    @method('DELETE')
                                    <input type="hidden" name="code_id" value="{{ $accounts->id }}">
                                    <button type="submit" class="text-blue-600 underline">
                                        <img class="w-4" src="{{ asset('img/icon/delete.png') }}" alt="Remover Acesso">
                                    </button>
                                </form>
                            </td>
                        </tr>
